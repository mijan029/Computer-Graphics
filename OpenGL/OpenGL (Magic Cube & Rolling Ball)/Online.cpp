...
double ghurakon = 0;
void drawNagordola(){
    double ghurakon = ghurakon + ttheta/20*agao;
    glPushMatrix();
    glRotatef(-ghurakon,0,0,1);
    glPushMatrix();
    //
    for(int i=0; i<6; i++){
        glPushMatrix();
...
void keyboard(unsigned char key, int x, int y) {
    ...
    switch (key) {
        ...
        case 'd':
            ...
            break;
         case '1':{
            if(ttheta<90) ttheta+=10;
			break;
         }

        case '2':{
            if(ttheta>0) ttheta-=10;
			break;
        }
